...
namespace UI {
    public class UIPlayerPosition : MonoBehaviour {
        ...
        public void OnUpdate() {
            if (!_cameraManager.IsSplit) {
                UpdatePositionFromCameraSingle(_cameraManager.SingleCam);
                _rectTransform.gameObject.SetActive(true);
            }
            else {
                var cameraToCheck = playerCamNumber == 1 ? _cameraManager.PlayerCam1 : _cameraManager.PlayerCam2;
                UpdatePositionFromCameraSplit(cameraToCheck);

                // the point is not visible if the player is behind the camera
                _rectTransform.gameObject.SetActive(math.dot(cameraToCheck.transform.forward, _playerToFollow.transform.position - cameraToCheck.transform.position) > 0f);
            }
        }
        ...
        private void UpdatePositionFromCameraSplit(Camera cam) {
            ...
            _rectTransform.localPosition = new Vector2(xPosition, yPosition) + Vector2.right * (sign * cam.pixelWidth) / 2f;
        }
    }
}

...
namespace Player
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        public void Advance(float directionInput, bool brakeInput)
        {
            ...
            if (math.distance(_splineNearestPoint, transform.position) >= maxSteerLength * 5)
                transform.position = _splineNearestPoint;

            _speedUI.UpdateSpeed(
                brakeInput ? 0 : actualSpeed,
                speed * boostSpeedMultiplier * (2 * proximityBoostMultiplier)
            );

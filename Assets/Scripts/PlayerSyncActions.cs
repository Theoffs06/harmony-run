...
public class PlayerSyncActions : MonoBehaviour
{
    ...
    private float _player1LastTrickTime = 0;
    private float _player2LastTrickTime = 1;
    private bool _player1MadeATrick = false;
    private bool _player2MadeATrick = false;

    public int SuceedTricks(int player)
    {
        var points = baseTrickScore;
        float tricksTimingDifference = -1;
        if (player == 0)
        {
        ...
        {
            tricksTimingDifference = Mathf.Abs(_player1LastTrickTime - _player2LastTrickTime);
